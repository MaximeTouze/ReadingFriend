...
df1 = pd.read_csv('./static/dataset/S08_question_answer_pairs.txt', sep='\t')
df2 = pd.read_csv('./static/dataset/S09_question_answer_pairs.txt', sep='\t')
df3 = pd.read_csv('./static/dataset/S10_question_answer_pairs.txt', sep='\t', encoding = 'ISO-8859-1')
frames = [df1, df2, df3]
df = pd.concat(frames)

def getArticleText(file):
    fpath = './static/dataset/text_data/'+file+'.txt.clean'
    try:
        f = open(fpath, 'r')
        text = f.read()
...
# Returns only contexts that are prone to contain the answer
def get_best_contexts(question, context_list):
    best_context = []
    sims = []
    best_context_number = 5

    for context in context_list:
        sims.append(getSim((str(question).split(), 0), context))

    for i in range(0, best_context_number):
        best_context.append(context_list[sims.index(max(sims))])
        context_list.pop(sims.index(max(sims)))
...
def getAnswerBert(question, context):
    ...
    best_context_list = get_best_contexts(question, context_list)

    ans = []

    for c in best_context_list:
...
context = """I. A SCANDAL IN BOHEMIA
...
#question="Where is Paris" # question with no answer from context
question = "What is the name of the main character" # question with an answer from context
